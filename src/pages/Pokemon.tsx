...
function Pokemon() {
  ...
  const getPokemonInfo = useCallback(
    async (image: string) => {
      const { data } = await axios.get(`${POKEMON_ROUTE}/${params.id}`);
      ...
      } = await axios.get(`${POKEMON_SPECIES_ROUTE}/${data.id}`);
  ...
  useEffect(() => {
    ...
    const options = {
      ...
      distance: 1,
      splitPower: 10,
      colorValidator: (_: number, _2: number, _3: number, alpha = 255) =>
        alpha > 250,
  ...
  return (
    <>
      {currentPokemonTab === POKEMON_TABS.description && <Description />}
      {currentPokemonTab === POKEMON_TABS.evolution && <Evolution />}
      {currentPokemonTab === POKEMON_TABS.moves && <CapableMoves />}
      {currentPokemonTab === POKEMON_TABS.locations && <Location />}
    </>
  );
}
